...
class ParseInsuranceQA(object):
    ...
    def build(cls, dpath):
        ...
        for dtype, data_fname in cls.data_fnames:
...
class ParseInsuranceQAV2(ParseInsuranceQA):
    ...
    # TODO: support 500, 1000, 1500 (# of label candidates)
    data_fnames = [
        ("train", "InsuranceQA.question.anslabel.token.100.pool.solr.train.encoded.gz"),
        ("valid", "InsuranceQA.question.anslabel.token.100.pool.solr.valid.encoded.gz"),
        ("test", "InsuranceQA.question.anslabel.token.100.pool.solr.test.encoded.gz")
    ]
